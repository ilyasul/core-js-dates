...
function getDayName(date) {
  ...
  return Intl.DateTimeFormat('en-us', options).format(test);
}
...
function getNextFriday(date) {
  ...
  const countDay = dayOfWeek < 5 ? 5 - dayOfWeek : 12 - dayOfWeek;
  const dateNew = new Date(date);
  dateNew.setDate(date.getDate() + countDay);
  return dateNew;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const countMS = Math.abs(start > end ? start - end : end - start) + 86400000;
  return countMS / (1000 * 60 * 60 * 24);
}
...
function isDateInPeriod(date, period) {
  ...
  return start <= testDate && end >= testDate;
}
